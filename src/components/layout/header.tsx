        ...
        <div className="flex items-center gap-4">
            ...
            <Sheet open={isMenuOpen} onOpenChange={setMenuOpen}>
              <SheetTrigger asChild>
                <Button variant="outline" size="icon" className="xl:hidden border-none bg-transparent hover:bg-secondary">
                  <Menu className="h-6 w-6" />
                  <span className="sr-only">Открыть меню</span>
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-full h-full p-0 flex flex-col border-none">
                <div className="container flex h-16 md:h-20 items-center justify-between border-b">
                ...
                <div className="flex-grow flex flex-col items-center justify-center gap-8">
                  {navLinks.map(({ href, label }) => (
                    <Link
                      key={href}
                      href={href}
                      className="text-3xl font-bold transition-colors hover:text-primary active:scale-95 duration-200"
                      onClick={(e) => handleLinkClick(e, href)}
                    >
                      {label}
                    </Link>
                  ))}
                  <Button asChild size="lg" className="mt-4 text-xl px-12 py-6 rounded-full shadow-lg">
                      <Link href="/#contact-form" onClick={(e) => handleLinkClick(e, '/#contact-form')}>Связаться</Link>
                  </Button>
                </div>
              </SheetContent>
